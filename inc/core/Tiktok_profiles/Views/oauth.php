            ...
            <div class="card mb-4 mb-xl-10">
                <div class="card-header cursor-pointer">
                    <div class="card-title m-0">
                        <h3 class="fw-bold m-0"><i class="<?php _e( $config['icon'] )?>" style="color: <?php _e($config['color'])?>"></i> <?php _e("Add Tiktok profiles")?></h3>
                    </div>
                </div>
                <div class="card-body">
                    <?php if (check_number_account("tiktok", "profile", false, false) || uri("segment", 3) == $instance_id): ?>
                    <div class="py-2 check-wrap-all">
                        <div class="border b-r-10 p-20 mb-4">
                            ...
                            <div class="text-gray-600"><?php _e("Scan the QR Code on your Tiktok app")?></div>
                        </div>

                        <div class="text-center tiktok-qr-code" data-instance-id="<?php _ec($instance_id)?>">
                            <div class="tiktok-code text-center">
                    ...
                    <?php else: ?>
                        ...
                        <div class="alert alert-danger d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <?php _e( sprintf(__("You can only add up to %s Tiktok profiles"), $number_accounts ) )?>
                            </div>
                        </div>
                    <?php endif ?>
                </div>
                        ...
                        <div class="d-flex flex-stack">
                            ...
                            <div class="d-flex align-items-center flex-row-fluid flex-wrap">
                                <div class="flex-grow-1 me-2 text-over-all">
                                    ...
                                    <a href="<?php _ec( base_url("tiktok_profiles/oauth/".$value->token) )?>" class="text-danger fw-semibold d-block fs-12"><?php _e( "Re-login required" )?></a>
                                    <?php endif ?>
                                </div>
                            </div>
                            <a href="<?php _ec( base_url("tiktok_profiles/oauth/".$value->token) )?>" class="btn btn-sm btn-outline-dashed btn-light-danger "><?php _e("Relogin")?></a>
                        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="note">
                        ...
                        <a href="<?php _ec( get_module_url("oauth") )?>" class="btn btn-outline btn-outline-dashed bg-white"><i class="<?php _ec( $config['icon'] )?>" style="color: <?php _ec( $config['color'] )?>"></i> <?php _e("Re-connect with Tiktok")?></a>
                    </div>
                </div>
            </div>
        </div>
